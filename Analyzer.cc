...
//Aman
  ...
  initializeMuonFakeFactor(infiles);
  ...
  for(auto lvec: lep) {
    ...
    for( auto cut: stats.bset) {
      ...
      /////muon cuts
        ...
        if(cut == "DoDiscrByTightID") passCuts = passCuts && _Muon->medium->at(i);
...
break;
  ...
  //note the leading jet has to be selected fist! //addition3
  if(ePos == CUTS::eR1stJet || ePos == CUTS::eR2ndJet) {
...
void Analyzer::initializeTauFakeFactor(std::vector<std::string> infiles){
 ...
 TFile hist_fake("Pileup/fakerate.root");
 fake_h =dynamic_cast<TH2F*>(hist_fake.FindObjectAny("h1"));
 hist_fake.Close();
}

/*
double Analyzer::FakeRate(){
        double fakerate = 1.;
	if(active_part->at(CUTS::eRTau1)->size()){

        double tauPt=_Tau->p4(active_part->at(CUTS::eRTau1)->at(0)).Pt();
	  fakerate = fake_h->GetBinContent(fake_h->FindBin(tauPt));
	cout<<"pt of tau    "<<tauPt<<endl;
	...
	double temp = 1.;
        ...
        double tauEta = _Tau->p4(active_part->at(CUTS::eRTau1)->at(0)).Eta();
				int binx = fake_h->GetXaxis()->FindBin(tauPt);
                         	...
                         	temp = fake_h->GetBinContent(binx,biny);
				fakerate = temp/(1-temp);
 

  //      cout<<"pt of tau    "<<tauPt<<endl;
//	cout<<"eta of tau    "<<tauEta<<endl;	
  //      cout<<"===============fake rate    "<<temp<<'\t'<<fakerate<<endl;
          }
          return fakerate;
          }
...
//Aman
void Analyzer::initializeMuonFakeFactor(std::vector<std::string> infiles){

 TFile hist_fakeMuon("Pileup/muon_fakerate.root");
 Muonfake_h =dynamic_cast<TH2F*>(hist_fakeMuon.FindObjectAny("fake_muon_h"));
 hist_fakeMuon.Close();
}

double Analyzer::MuonFakeRate(){
                double Muonfakerate = 1.;
		double temp1 = 1.;
                        if(active_part->at(CUTS::eRMuon1)->size()){
        
                                double muPt = _Muon->p4(active_part->at(CUTS::eRMuon1)->at(0)).Pt();
                                double muEta = _Muon->p4(active_part->at(CUTS::eRMuon1)->at(0)).Eta();
				int binx = Muonfake_h->GetXaxis()->FindBin(muPt);
				int biny = Muonfake_h->GetYaxis()->FindBin(muEta);
				temp1 = Muonfake_h->GetBinContent(binx,biny);
                                Muonfakerate = temp1/(1-temp1);                  
				
//				cout<<"pt of mu    "<<muPt<<endl;
//				  cout<<"Eta of mu    "<<muEta<<endl;
						
						
   //                            cout<<"=============fake rate    "<<temp1<<'\t'<<Muonfakerate<<endl;
...
//Aman
   if(isData){
           wgt = 1.;
                    if(distats["Run"].bfind("ApplyTauFakeRate")) wgt *= FakeRate();
	if(distats["Run"].bfind("ApplyMuonFakeRate")) wgt *= MuonFakeRate(); 
//	cout<<"weight "<<wgt<<endl;
  ...
  backup_wgt=wgt;
	//cout<<"Final Weight  "<<backup_wgt<<endl;
...
//addition2
 double minDetlaPhi=99;
    int closest_jet=-1;
    for(int ijet : *active_part->at(CUTS::eRJet1)){
      double thisDetlaPhi= absnormPhi(_Jet->p4(ijet).DeltaPhi(_MET->p4()));
      if(thisDetlaPhi<minDetlaPhi){
        minDetlaPhi=thisDetlaPhi;
        closest_jet=ijet;
      }
    }
    if(minDetlaPhi<90){
      histAddVal(minDetlaPhi,"DeltaPhiJetMet");
      if (minDetlaPhi<0.5){
        histAddVal(_MET->pt()/_Jet->pt(closest_jet),"MetJetRatio");
      }
    }
...
//cout<<"leading Jet pt = "<<_Jet->p4(active_part->at(CUTS::eR1stJet)->at(0)).Pt()<<endl;
//cout<<"leading Jet eta = "<<_Jet->p4(active_part->at(CUTS::eR1stJet)->at(0)).Eta()<<endl;
//cout<<"==========Pair STOP==========="<<endl;

histAddVal2(_Jet->p4(active_part->at(CUTS::eR1stJet)->at(0)).Pt(), _Jet->p4(active_part->at(CUTS::eR1stJet)->at(0)).Eta(), "FirstPtVsFirstEta"); 
}
